...
from chanlib.base import (
    ...
    NIGHT_MARKET_SQ2,
    MARKET_ZJ)
from chanlib.cta_period import CtaPeriod, Period
from vnpy.trader.object import BarData, TickData
from vnpy.trader.constant import Interval
from vnpy.trader.utility import round_to
from chanlib.compat import Color, ChanSignals, get_trading_date, get_underlying_symbol
from chanlib.cta_utility import check_chan_xt, check_chan_xt_three_bi, check_qsbc_2nd

try:
    from chanlib.chanlun import ChanGraph, ChanLibrary
except Exception as ex:
    print('can not import pyChanlun from chanlib.chanlun')


def get_interval_type(bar_name:str):
    """
    获取bar的类型与步长
    :param bar_name: M15 =》 Interval.MINUTE, 15
    :return:
    """
    if bar_name.startswith('S'):
        return Interval.SECOND, int(bar_name.replace('S', ''))

    if bar_name.startswith('M'):
        return Interval.MINUTE, int(bar_name.replace('M', ''))

    if bar_name.startswith('H'):
        return Interval.HOUR, int(bar_name.replace('H', ''))
    ...
    if bar_name.startswith('D'):
        ...
        if len(interval) == 0:
            ...
            return Interval.DAILY, int(interval)

    if bar_name.startswith('W'):
        interval = bar_name.replace('W', '')
        if len(interval) == 0:
            return Interval.WEEKLY, 1
        else:
            return Interval.WEEKLY, int(interval)
...
class CtaLineBar(object):
    ...
    def __init__(self, strategy, cb_on_bar, setting=None):
        ...
        # 当前的Tick的信息
        self.cur_tick:TickData = None  # 当前 onTick()函数接收的 最新的tick
        self.last_tick:TickData = None  # 当前正在合成的 X周期bar 的最后(新)一根tick
        ...
        self.export_bi_trends_filename = None  # 缠论笔走势的csv文件, stratgy_name_bi_trends.csv

        # n 会转换为 3，5，7，9，11，13

        self.pre_bi_start = None  # 前一个笔的时间
        self.pre_zs_start = None  # 前一个中枢的时间
        self.pre_duan_start = None # 前一个段的时间
        self.pre_bi_trend_start = None # 前一个笔形态走势的时间
    ...
    def init_param_list(self):
        ...
        self.param_list.append('para_emaadx_len')
        ...
        self.param_list.append('para_active_yb2')  # 重心线
        self.param_list.append('para_yb2_len')
        self.param_list.append('para_yb2_ref')
        ...
        self.param_list.append('para_active_chan_trend')  # 激活缠论的走势分析
    ...
    def init_indicators(self):
        ...
        self.para_emaadx_len = 0  # 14           # ADX的计算周期
        ...
        self.para_active_yb2 = False  # 是否激活多空趋势线
        self.para_yb2_ref = 1  # 趋势线参照周期
        self.para_yb2_len = 10  # 趋势线观测周期
        ...
        self.line_adx_dx = []
        self.line_emaadx = []
        self.cur_emaadx = 0  # Bar内计算的平均趋向指标
        ...
        self.xt_3_signals = []  # czsc 三笔信号列表  {'start'最后一笔开始,'end'最后一笔结束,'signal':ChanSignal信号,price:价格}
        self.xt_5_signals = []  # czsc 五笔信号列表  {'start'最后一笔开始,'end'最后一笔结束,'signal'}
        self.xt_7_signals = []  # czsc 七笔信号列表  {'start'最后一笔开始,'end'最后一笔结束,'signal'}
        self.xt_9_signals = []  # czsc 九笔信号列表  {'start'最后一笔开始,'end'最后一笔结束,'signal'}
        self.xt_11_signals = []  # czsc 11笔信号列表  {'bi_start'最后一笔开始,'end'最后一笔结束,'signal'}
        self.xt_13_signals = []  # czsc 13笔信号列表  {'bi_start'最后一笔开始,'end'最后一笔结束,'signal'}
        self.xt_2nd_signals = []  # 趋势背驰2买或趋势背驰2卖信号

        self.para_active_chan_trend = False  # 是否激活缠论走势识别（上升盘整、下降盘整、上升趋势、下降趋势）
        self._last_bi = None    # 识别时，最后一笔
        self.bi_trends = []     # 由笔中枢构成得走势识别
        self.cur_bi_trend = None
        self._last_duan = None  # 识别时，最后一线段
        self.duan_trends = []   # 由段中枢构成得走势识别
        self.cur_duan_trend = None
    ...
    def on_tick(self, tick: TickData):
        ...
        if tick.last_volume ==0:
            if self.cur_tick:
                if self.cur_tick.trading_day != tick.trading_day:
                    tick.last_volume = tick.volume
                else:
                    tick.last_volume = max(tick.volume - self.cur_tick.volume,1)
    ...
    def get_last_bar_str(self):
        ...
        if self.para_emaadx_len > 0 and len(self.line_emaadx) > 0:
            msg = msg + u',Pdi:{1};Mdi:{1};emaAdx:{2}'.format(self.line_adx_pdi[-1], self.line_adx_mdi[-1], self.line_emaadx[-1])
        ...
        if self.para_active_yb2 and len(self.line_yb2) > 1:
            c = 'Blue2' if self.line_yb2[-1] < self.line_yb2[-2] else 'Yellow2'
            msg = msg + u',YB2:{},[{}({})]'.format(self.line_yb2[-1], c, self.cur_yb2_count)
...
    ·在10周期内当价格方向不明确的时候，自适应均线应该是横向移动；
    ...
    def __count_dmi(self):
        ...
        if self.cur_pdi > self.cur_mdi and self.cur_adx_trend and self.cur_adxr_trend and self.cur_pdi >= self.para_dmi_max:
            ...
            self.write_log(u'{0}[DEBUG]Buy Signal On Bar,Pdi:{1}>Mdi:{2},adx[-1]:{3}>Adx[-2]:{4}'
                           .format(self.cur_datetime, self.cur_pdi, self.cur_mdi, self.line_adx[-1],
                                   self.line_adx[-2]))
        else:
    ...
    def __count_emaadx(self):
        """计算K线的DMI数据和条件"""

        if self.para_emaadx_len <= 0:  # 不计算
            return

        # 1、lineMx满足长度才执行计算
        if len(self.line_bar) < self.para_emaadx_len + 1:
            self.write_log(u'数据未充分,当前Bar数据数量：{0}，计算ADX需要：{1}'.format(len(self.line_bar), self.para_emaadx_len * 2 + 1))
            return

        # 2、根据当前High，Low，(不包含当前周期）重新计算TR1
        count_len = min(self.bar_len - 1, self.para_emaadx_len)
        cur_atr1 = ta.ATR(self.high_array[-count_len * 2:], self.low_array[-count_len * 2:],
                          self.close_array[-count_len * 2:], count_len)[-1]
        cur_atr1 = round(cur_atr1, self.round_n)
        if len(self.line_adx_atr1) > self.max_hold_bars:
            del self.line_adx_atr1[0]
        self.line_adx_atr1.append(cur_atr1)

        # 今高与昨高的价差
        high_prehigh_spread = self.line_bar[-1].high_price - self.line_bar[-2].high_price
        # 昨低与今低的价差
        low_prelow_spread = self.line_bar[-2].low_price - self.line_bar[-1].low_price

        # 3、计算周期内的做多价差
        if high_prehigh_spread > 0 and high_prehigh_spread > low_prelow_spread:
            self.line_adx_hd1.append(high_prehigh_spread)
        else:
            self.line_adx_hd1.append(0)

        # 4、计算周期内的做空价差
        if low_prelow_spread > 0 and low_prelow_spread > high_prehigh_spread:
            self.line_adx_ld1.append(low_prelow_spread)
        else:
            self.line_adx_ld1.append(0)

        if len(self.line_adx_hd1) < 2:
            return

        # 5、计算上升动向指标，即做多的比率
        cur_dmp1 = ta.EMA(np.array(self.line_adx_hd1, dtype=float), min(len(self.line_adx_hd1), count_len))[-1]
        if np.isnan(cur_dmp1):
            return
        cur_pdi1 = cur_dmp1 * 100 / cur_atr1
        cur_pdi1 = round(float(cur_pdi1), self.round_n)
        if len(self.line_adx_pdi) > self.max_hold_bars:
            del self.line_adx_pdi[0]
        self.line_adx_pdi.append(cur_pdi1)

        # 6、计算下降动向指标，即做空的比率
        cur_dmm1 = ta.EMA(np.array(self.line_adx_ld1, dtype=float), min(len(self.line_adx_ld1), count_len))[-1]
        if np.isnan(cur_dmm1):
            return
        cur_mdi1 = cur_dmm1 * 100 / cur_atr1
        cur_mdi1 = round(float(cur_mdi1), self.round_n)
        if len(self.line_adx_mdi) > self.max_hold_bars:
            del self.line_adx_mdi[0]
        self.line_adx_mdi.append(cur_mdi1)

        # 7、计算平均趋向指标 Adx，Adxr
        if cur_mdi1 + cur_pdi1 == 0:
            dx = 0
        else:
            dx = 100 * abs(cur_mdi1 - cur_pdi1) / (cur_mdi1 + cur_pdi1)

        if len(self.line_adx_dx) > self.max_hold_bars:
            del self.line_adx_dx[0]

        self.line_adx_dx.append(dx)

        # 平均趋向指标，MA计算
        if len(self.line_adx_dx) < self.para_emaadx_len + 1:
            self.cur_emaadx = dx
        else:
            self.cur_emaadx = ta.EMA(np.array(self.line_adx_dx, dtype=float), min(len(self.line_adx_dx), count_len))[-1]
        if np.isnan(self.cur_emaadx):
            self.cur_emaadx = 0
            return

        # 保存Adx值
        if len(self.line_emaadx) > self.max_hold_bars:
            del self.line_emaadx[0]

        self.line_emaadx.append(self.cur_emaadx)
...
 
    ...
    def __count_yb2(self):
        """某种趋势线"""

        if not self.para_active_yb2:
            return

        if self.para_yb2_len < 1:
            return

        if self.para_yb2_ref < 1:
            self.write_log(u'参数 self.inputYb2Ref:{}不能低于1'.format(self.para_yb2_ref))
            return

        # 1、lineBar满足长度才执行计算
        # if len(self.lineBar) < 4 * self.inputYb2Len:
        #    self.write_log(u'数据未充分,当前Bar数据数量：{0}，计算YB2 需要：{1}'.
        #                     format(len(self.lineBar), 4 * self.inputYb2Len))
        #    return

        ema_len = min(self.bar_len - 1, self.para_yb2_len)
        if ema_len < 3:
            self.write_log(u'数据未充分,当前Bar数据数量：{0}'.
                           format(len(self.line_bar)))
            return
        ...
        if len(self.line_yb2) > self.max_hold_bars:
            del self.line_yb2[0]

        self.line_yb2.append(bar_mid3_ema10)

        if len(self.line_yb2) < self.para_yb2_ref + 1:
            return

        if self.line_yb2[-1] > self.line_yb2[-1 - self.para_yb2_ref]:
            self.cur_yb2_count = self.cur_yb2_count + 1 if self.cur_yb2_count >= 0 else 1
        else:
            self.cur_yb2_count = self.cur_yb2_count - 1 if self.cur_yb2_count <= 0 else -1

    def rt_count_yb2(self):
        """
        实时计算黄蓝
        :return:
        """
        ...
        ema_len = min(len(self.line_bar), self.para_yb2_len)
        if ema_len < 3:
            self.write_log(u'数据未充分,当前Bar数据数量：{0}'.
                           format(len(self.line_bar)))
            return
        ...
        last_bar_mid3 = (self.line_bar[-1].close_price + self.line_bar[-1].high_price + self.line_bar[-1].low_price) / 3
        bar_mid3_ema10 = ta.EMA(np.append(self.mid3_array[-ema_len * 4:], [last_bar_mid3]), ema_len)[-1]
        self._rt_yb2 = round(float(bar_mid3_ema10), self.round_n)

    @property
    def rt_yb2(self):
        self.check_rt_funcs(self.rt_count_yb2)
        if self._rt_yb2 is None and len(self.line_yb2) > 0:
            return self.line_yb2[-1]
        return self._rt_yb2
    ...
    def _get_list(self, obj_list, s_start=None):
        """
        内部函数，从对象列表中获取其start字段之后的对象
        :param obj_list:
        :param s_start:
        :return:
        """
        ...
        for i in range(1, len(obj_list)):
            if obj_list[-i].start <= s_start:
                break
            n += 1
        return obj_list[-n:] if n > 0 else []
    ...
    def export_chan(self):
        ...
        if self.export_bi_filename:
            ...
            bi_list = self._get_list(self.bi_list[:-1],self.pre_bi_start)
            for bi in bi_list:
                self.append_data(
        ...
        if self.export_zs_filename:
            ...
            zs_list = self._get_list(self.bi_zs_list[:-1], self.pre_zs_start)
            for zs in zs_list:
                self.append_data(
        ...
        if self.export_duan_filename:
            ...
            duan_list = self._get_list(self.duan_list[:-1], self.pre_duan_start)
        ...
        if self.export_bi_trends_filename:
            # csv 文件 "start", "end", "direction", "height", "high", "low"
            # 获取最后记录的start 开始时间
            if self.pre_bi_trend_start is None:
                self.pre_bi_trend_start = self.get_csv_last_dt(self.export_bi_trends_filename, dt_index=0)
                if isinstance(self.pre_bi_trend_start, datetime):
                    self.pre_bi_trend_start = self.pre_bi_trend_start.strftime("%Y-%m-%d %H:%M:%S")

            # 获取所有未写入文件的走势
            trends = [t for t in self.bi_trends[:-1] if (not self.pre_bi_trend_start) or t["start"] > self.pre_bi_trend_start]
            for t in trends:
                self.append_data(
                    file_name=self.export_bi_trends_filename,
                    dict_data=t,
                    field_names=["start", "end", "direction", "type", "high", "low","pre_type","zs_start","zs_confirm","zs_exit", "history"]
                )
                self.pre_bi_trend_start = t['start']

            if len(self.bi_trends) > 20:
                self.bi_trends =  self.bi_trends[-20:]
    ...
    def is_zs_beichi(self, direction: Direction, last_bi_end: str):
        """
        判断当前中枢是否满足中枢背驰
        :param direction:与分笔走势相反得背驰，例如last_bi_end得一笔是向上得，就判断是否顶背驰
        :param last_bi_end:当前中枢得离开笔
        :return:
        """
        # 无中枢，或者中枢方向，与判断方向不一致，都返回False
        if self.cur_bi_zs is None or self.cur_bi_zs.direction != direction:
            return False
        ...
        for i in range(1,len(self.bi_list)):
            if exit_bi is None and self.bi_list[-i].start == self.cur_bi_zs.end \
                    and self.bi_list[-i].direction == self.cur_bi_zs.direction:
                exit_bi = self.bi_list[-i]
            if entry_bi is None and self.bi_list[-i].end == self.cur_bi_zs.start:
                entry_bi = self.bi_list[-i]
                break

        if exit_bi is None or entry_bi is None:
            return False

        if exit_bi.end != last_bi_end:
            return False

        # 离开笔斜率，比进入笔小，形成背驰
        if exit_bi.atan < entry_bi.atan:
            return True
    ...
    def get_xt_signal(self, xt_name, x=0):
        """
        获取n笔形态/信号的倒x笔结果
        :param xt_name:xt_3_signals ~ xt_13_signals , xt_2nd_signals
        :param x: 倒x笔，如倒1笔, 倒0笔
        :return: {} 或 {'start': 最后一笔开始,'end':最后一笔结束,'signal':ChanSignal信号, 'price':价格}
        """
    ...
    def update_bi_trend(self):
        """
        更新缠论分笔的走势形态识别
        :return:
        """
        if not self.para_active_chan_trend:
            return

        # 当前笔的start == 上一次判断过得
        if self.cur_bi is None or self.cur_bi.start == self._last_bi:
            return

        # 当前笔走势识别
        if len(self.bi_trends) > 0:
            self.cur_bi_trend = self.bi_trends[-1]

        # 当前没有笔走势识别
        if self.cur_bi_trend is None:

            # 具有笔中枢
            if self.cur_bi_zs:
                self._last_bi = self.cur_bi
                entry_bi = [bi for bi in self.bi_list if bi.end == self.cur_bi_zs.start]
                if len(entry_bi) == 0:
                    return
                entry_bi = entry_bi[0]
                self.cur_bi_trend = {
                    'type': '上升盘整' if self.cur_bi_zs.direction == 1 else '下降盘整',
                    'direction': self.cur_bi_zs.direction,
                    'start': entry_bi.start,
                    'end': self.bi_list[-1].end,
                    'zs_start': self.cur_bi_zs.start,
                    'high': max(entry_bi.high, self.cur_bi_zs.max_high, self.cur_bi.high),
                    'low': min(entry_bi.low, self.cur_bi_zs.min_low, self.cur_bi.low)
                }
                self.bi_trends.append(copy.copy(self.cur_bi_trend))

            # 还没有中枢，只有线段
            else:
                if self.cur_duan and len(self.cur_duan.bi_list) >= 7:
                    self._last_bi = self.cur_bi
                    self.cur_bi_trend = {
                        'type': '上升趋势' if self.cur_duan.direction == 1 else '下降趋势',
                        'direction': self.cur_duan.direction,
                        'start': self.cur_duan.start,
                        'end': self.bi_list[-1].end,
                        'zs_start': "",
                        'high': self.cur_duan.high,
                        'low': self.cur_duan.low
                    }
        ...
        # 当前存在识别的走势
            ...
            # 出现新得中枢,可能性（上升为例）
            # 上升盘整 => 上升趋势；上升盘整或上升趋势 => 中枢扩张； 上升盘整 => 下降盘整
            if self.cur_bi_zs and self.cur_bi_zs.start > self.cur_bi_trend['zs_start']:
                self._last_bi = self.cur_bi
                # 新得中枢方向，与当前形态得方向一致, 盘整 => 趋势； 或者 中枢扩张
                if self.cur_bi_zs.direction == self.cur_bi_trend['direction']:
                    if len(self.bi_zs_list) >= 2:
                        self._last_bi = self.cur_bi
                        # 前后两个中枢不能重叠:  盘整 => 趋势
                        if (self.cur_bi_zs.direction == 1 and self.cur_bi_zs.min_low >
                            self.bi_zs_list[-2].max_high) \
                                or \
                                (self.cur_bi_zs.direction == -1 and self.cur_bi_zs.max_high <
                                 self.bi_zs_list[-2].min_low):
                            self.cur_bi_trend['pre_type'] = self.cur_bi_trend['type']
                            self.cur_bi_trend['type'] = self.cur_bi_trend['type'].replace('盘整', '趋势').replace("-中枢扩张","")
                        # 重叠的话，变成中枢扩张
                        else:
                            if not self.cur_bi_trend['type'].endswith('扩张'):
                                self.cur_bi_trend['pre_type'] = self.cur_bi_trend['type']
                                self.cur_bi_trend['type'] = self.cur_bi_trend['type'] + "-中枢扩张"

                    # 更新新的中枢开始时间
                    self.cur_bi_trend['zs_start'] = self.cur_bi_zs.start
                    # 剔除原中枢的确认记录
                    self.cur_bi_trend.pop('zs_confirm', None)
                    # 剔除原中枢的退出记录
                    self.cur_bi_trend.pop('zs_exit', None)

                    # 添加历史记录
                    history = self.cur_bi_trend.get('history', '['+self.cur_bi_trend['start'] + ']' + self.cur_bi_trend['type'])
                    self.cur_bi_trend['history'] = history + ";[" + self.cur_bi_trend['zs_start'] + ']' + self.cur_bi_trend['type']

                # 新得中枢方向，与当前形态方向不一致，=》 盘整
                else:
                    self._last_bi = self.cur_bi
                    entry_bi = [bi for bi in self.bi_list if bi.end == self.cur_bi_zs.start]
                    if len(entry_bi) == 0:
                    ...
                    # 判断前面的状态是否需要部分回滚
                    pre_bi_trend = self.bi_trends[-2]
                    if len(self.bi_zs_list) >=2 and pre_bi_trend.get('pre_type', None) and pre_bi_trend['zs_start'] > self.bi_zs_list[-2].start:
                        pre_bi_trend['type'] = pre_bi_trend['pre_type']
                        pre_bi_trend.pop('pre_type')
                        pre_bi_trend['zs_start'] = self.bi_zs_list[-2].start
                        pre_bi_trend['history'] += '[' + self._last_bi.start +']回滚-' + pre_bi_trend['type']

            # 存在消失中枢情况，如果盘整时中枢消失，整个走势都需要回滚清除
            if self.cur_bi_trend['type'] in ['上升盘整','下降盘整'] \
                and self.cur_bi_zs and self.cur_bi_zs.start < self.cur_bi_trend['zs_start']:
                # 移除最后一个走势，回滚到上一个
                self.bi_trends.pop(-1)
                return

            # 对于当前中枢，确认中枢结束，有两种情况，就是三类买卖点
            if self.cur_bi_trend['direction'] == 1:
                # 没出现中枢确认记录，当出现三买信号时，上升中枢确认
                if self.cur_bi_trend.get('zs_confirm',None) is None \
                        and self.cur_bi.direction == 1 \
                        and self.cur_bi_zs \
                        and self.cur_bi.low > self.cur_bi_zs.high:
                    # 当前上升中枢确认的时间，是三类买点
                    self.cur_bi_trend['zs_confirm'] = self.cur_bi.start
                if len(self.bi_list) > 4 :
                    # 没有出现上升中枢的三卖信号时，出现三卖信号，上升中枢结束，预示下跌信号的可能性较高
                    if self.cur_bi_trend.get('zs_exit',None) is None \
                            and self.cur_bi.direction == -1 \
                            and self.cur_bi.high < self.bi_list[-4].low:
                        self.cur_bi_trend['zs_exit'] = self.cur_bi.start
                    # 重新出现三买信号时，去除中枢结束信号
                    if self.cur_bi.direction == 1 \
                            and self.cur_bi_trend.get('zs_exit', None) == self.bi_list[-4].start \
                            and self.cur_bi.low > self.bi_list[-4].high:
                        self.cur_bi_trend.pop('zs_exit', None)

            if self.cur_bi_trend['direction'] == -1:
                # 没出现中枢确认记录，当出现三卖信号时，下降中枢确认
                if self.cur_bi_trend.get('zs_confirm', None) is None \
                        and self.cur_bi.direction == -1 \
                        and self.cur_bi_zs \
                        and self.cur_bi.high < self.cur_bi_zs.low:
                    # 当前中枢确认的时间，是三类买点
                    self.cur_bi_trend['zs_confirm'] = self.cur_bi.start
                # 没有出现中枢的三买信号时，出现三买信号，下降中枢结束，预示上升信号的可能性较高
                if self.cur_bi_trend.get('zs_exit', None) is None \
                        and self.cur_bi.direction == 1 \
                        and self.cur_bi.low > self.bi_list[-4].high:
                    self.cur_bi_trend['zs_exit'] = self.cur_bi.start
            ...
            # 更新最高、最低价、结束时间
            self.cur_bi_trend['high'] = max(self._last_bi.high, self.cur_bi_trend['high'])
            self.cur_bi_trend['low'] = min(self._last_bi.low, self.cur_bi_trend['low'])
            self.cur_bi_trend['end'] = self._last_bi.end
    ...
    def bar_height_ma(self,bar_len=20):
        """
        最近若干bar的高度平均值
        :param bar_len:
        :return:
        """
        heights = [bar.high_price - bar.low_price for bar in self.line_bar[-bar_len:-1]]
    ...
    def is_bars_reverse(self,n_bars=1,r_bars=1, direction=Direction.LONG):
        """
        是否满足n根bar反包rev_bars, 如 1阳包3阴， 2阳包3阴等
        :param n_bars: 满足direction方向的bar数目
        :param r_bars: 满足被反包的bar数目
        :param direction: 判断方向，Direction.LONG,阳线，Direction.SHORT, 阴线
        :return: True，满足
        """
        if n_bars+r_bars > self.bar_len:
            return False

        # 判断阳包阴
        if direction == Direction.LONG:
            long_bars = self.line_bar[-n_bars-1:-1]
            short_bars = self.line_bar[-n_bars - r_bars-1:-n_bars-1]
            # 都是阳线
            n_match = all([bar.close_price > bar.open_price for bar in long_bars])
            # 存在阴线即可
            r_match = any([bar.close_price < bar.open_price for bar in short_bars])
            if not all([n_match, r_match]):
                return False
            short_high = max([bar.high_price for bar in short_bars])
            short_low = min([bar.close_price for bar in short_bars])
            # 是否满足阳线反包阴线
            if long_bars[-1].close_price > short_bars[0].open_price \
                and long_bars[0].low_price < short_low \
                and long_bars[-1].close_price - long_bars[0].low_price > short_high - short_low:
                return True

        # 判断阴包阳
        else:
            short_bars = self.line_bar[-n_bars - 1:-1]
            long_bars = self.line_bar[-n_bars - r_bars - 1:-n_bars - 1]
            # 都是阴线
            n_match = all([bar.close_price < bar.open_price for bar in short_bars])
            # 存在阳线即可
            r_match = any([bar.close_price > bar.open_price for bar in long_bars])
            if not all([n_match, r_match]):
                return False
            long_low = min([bar.low_price for bar in long_bars])
            long_high = max([bar.close_price for bar in long_bars])
            # 是否满足阴线反包阳线
            if short_bars[-1].close_price < long_bars[0].open_price \
                and short_bars[0].high_price > long_high \
                and short_bars[0].high_price - short_bars[-1].close_price > long_high - long_low:
                return True
    ...
    def get_data(self):
        ...
        # ADX动能指标（附图）
        if isinstance(self.para_emaadx_len, int) and self.para_emaadx_len > 0:
            indicator = {
                'name': 'ADXEMA({})'.format(self.para_emaadx_len),
                'attr_name': 'line_emaadx',
                'is_main': False,
                'type': 'line'}
            indicators.update({indicator.get('name'): copy.copy(indicator)})
...
class CtaMinuteBar(CtaLineBar):
    ...
    def add_bar(self, bar, bar_is_completed=False, bar_freq=1):
        ...
        if last_bar.trading_day < bar.trading_day:
            is_new_bar = True
        ...
        if is_new_bar:
            ...
            last_bar.high_price = max(last_bar.high_price, bar.high_price)
            last_bar.low_price = min(last_bar.low_price, bar.low_price)
            last_bar.volume = last_bar.volume + bar.volume
            last_bar.open_interest = bar.open_interest
    ...
    def generate_bar(self, tick):
        ...
        if len(tick.trading_day) > 0 and len(self.cur_trading_day) > 0 and self.cur_trading_day < tick.trading_day:
            ...
            if self.cur_trading_day == "2022-08-19":
                a = 1
            self.bars_count = bars_passed
